"use client";
import { useState } from "react";
import { useAgentReasoning, REASONING_CHAINS } from "@/components/AgentReasoning";
import MathRenderer from "@/components/MathRenderer";
import ReactMarkdown from "react-markdown";

const phaseColors: Record<string, { color: string; label: string; icon: string }> = {
  decomposition: { color: "#06b6d4", label: "Decomposition", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
  "tool-thinking": { color: "#f59e0b", label: "Tool Call", icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" },
  critique: { color: "#ef4444", label: "Self-Critique", icon: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" },
  synthesis: { color: "#10b981", label: "Synthesis", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
};

function renderContent(text: string) {
  // Split by LaTeX delimiters
  const parts = text.split(/(\$\$[\s\S]+?\$\$|\$[^$]+?\$)/g);
  return parts.map((part, i) => {
    if (part.startsWith("$$") && part.endsWith("$$")) {
      return <MathRenderer key={i} tex={part.slice(2, -2)} display />;
    }
    if (part.startsWith("$") && part.endsWith("$")) {
      return <MathRenderer key={i} tex={part.slice(1, -1)} />;
    }
    return <ReactMarkdown key={i}>{part}</ReactMarkdown>;
  });
}

export default function AgentReasoningPanel({ agentId }: { agentId: string }) {
  const availableChains = Object.entries(REASONING_CHAINS)
    .filter(([, chain]) => chain.agentId === agentId)
    .map(([key, chain]) => ({ key, prompt: chain.prompt }));

  const [selectedChain, setSelectedChain] = useState(availableChains[0]?.key ?? "");
  const [customPrompt, setCustomPrompt] = useState("");
  const [streamOutput, setStreamOutput] = useState("");
  const [streaming, setStreaming] = useState(false);

  const { chain, currentStep, isRunning, streamText, completedSteps, startReasoning, reset } =
    useAgentReasoning(selectedChain);

  async function handleCustomReason() {
    if (!customPrompt.trim()) return;
    setStreaming(true);
    setStreamOutput("");
    try {
      const res = await fetch(`/api/agents/${agentId}/reason`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: customPrompt }),
      });
      if (!res.body) return;
      const reader = res.body.getReader();
      const decoder = new TextDecoder();
      while (true) {
        const { done, value } = await reader.read();
        if (done) break;
        const text = decoder.decode(value);
        for (const line of text.split("\n")) {
          if (line.startsWith("data: ")) {
            const payload = line.slice(6);
            if (payload === "[DONE]") break;
            try {
              const data = JSON.parse(payload);
              if (data.text) setStreamOutput((prev) => prev + data.text);
            } catch { /* ignore */ }
          }
        }
      }
    } catch { /* ignore */ }
    setStreaming(false);
  }

  if (availableChains.length === 0 && !agentId) return null;

  return (
    <div className="glass-card p-6 space-y-4">
      <h2 className="text-lg font-semibold flex items-center gap-2">
        <svg className="w-5 h-5 text-[var(--accent-indigo)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        Live Reasoning
      </h2>

      {availableChains.length > 0 && (
        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Reasoning Chain</label>
            <select
              value={selectedChain}
              onChange={(e) => { setSelectedChain(e.target.value); reset(); }}
              className="mt-1 w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] outline-none"
            >
              {availableChains.map((c) => (
                <option key={c.key} value={c.key}>{c.prompt.slice(0, 80)}...</option>
              ))}
            </select>
          </div>

          {chain && (
            <div className="p-3 rounded-lg bg-[var(--bg-primary)] border border-[var(--border-primary)]">
              <p className="text-xs font-mono text-[var(--accent-indigo)]">{chain.prompt}</p>
            </div>
          )}

          <div className="flex gap-2">
            <button
              onClick={isRunning ? reset : startReasoning}
              className={`px-4 py-2 rounded-lg text-xs font-medium text-white ${isRunning ? "bg-[var(--accent-rose)]" : "bg-[var(--accent-indigo)]"} hover:opacity-90 transition-opacity`}
            >
              {isRunning ? "⏹ Stop" : "▶ Run Reasoning Chain"}
            </button>
            {!isRunning && completedSteps.length > 0 && (
              <button onClick={reset} className="px-4 py-2 rounded-lg text-xs font-medium bg-[var(--bg-elevated)] text-[var(--text-muted)] hover:bg-[var(--bg-card)] transition-colors">
                Reset
              </button>
            )}
          </div>

          {(isRunning || completedSteps.length > 0) && chain && (
            <div className="space-y-3">
              {chain.steps.map((step, idx) => {
                const phase = phaseColors[step.phase];
                const isComplete = completedSteps.includes(idx);
                const isCurrent = currentStep === idx && isRunning;
                const isPending = idx > currentStep || (!isRunning && !isComplete);
                return (
                  <div
                    key={idx}
                    className={`rounded-xl border p-4 transition-all ${isPending ? "opacity-30" : "opacity-100"}`}
                    style={{ borderColor: isComplete || isCurrent ? `${phase.color}40` : "var(--border-primary)", backgroundColor: isComplete || isCurrent ? `color-mix(in srgb, ${phase.color} 5%, transparent)` : "transparent" }}
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: `${phase.color}20` }}>
                        {isComplete ? (
                          <svg className="w-3.5 h-3.5" style={{ color: phase.color }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                          </svg>
                        ) : isCurrent ? (
                          <span className="w-2 h-2 rounded-full status-pulse" style={{ backgroundColor: phase.color }} />
                        ) : (
                          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: phase.color, opacity: 0.3 }} />
                        )}
                      </div>
                      <span className="text-xs font-semibold" style={{ color: phase.color }}>{phase.label}</span>
                      {step.tool && (
                        <span className="badge bg-[var(--bg-elevated)] text-[var(--text-muted)]" style={{ fontSize: 9 }}>{step.tool}</span>
                      )}
                      {isComplete && step.duration && (
                        <span className="text-[10px] text-[var(--text-muted)] ml-auto">{step.duration}ms</span>
                      )}
                    </div>
                    {(isComplete || isCurrent) && (
                      <div className="text-xs text-[var(--text-secondary)] font-mono leading-relaxed pl-8 space-y-1 max-h-48 overflow-y-auto">
                        <div className="prose prose-sm prose-invert max-w-none">
                          {renderContent(isCurrent ? streamText : step.content)}
                        </div>
                      </div>
                    )}
                  </div>
                );
              })}

              {completedSteps.length === chain.steps.length && (
                <div className="p-4 rounded-xl border border-[var(--accent-emerald)]/30 bg-[var(--accent-emerald)]/5">
                  <div className="flex items-center gap-2 mb-2">
                    <svg className="w-4 h-4 text-[var(--accent-emerald)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-xs font-semibold text-[var(--accent-emerald)]">Final Answer</span>
                    <span className="text-xs font-mono text-[var(--accent-emerald)] ml-auto">{chain.confidence}% confidence</span>
                  </div>
                  <p className="text-xs text-[var(--text-primary)] pl-6">{chain.finalAnswer}</p>
                  <p className="text-[10px] text-[var(--text-muted)] pl-6 mt-1">Verified via: {chain.verificationMethod}</p>
                </div>
              )}
            </div>
          )}
        </div>
      )}

      <div className="border-t border-[var(--border-primary)] pt-4 space-y-3">
        <label className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Custom Reasoning Prompt (AI-powered)</label>
        <div className="flex gap-2">
          <input
            type="text"
            value={customPrompt}
            onChange={(e) => setCustomPrompt(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleCustomReason()}
            placeholder={`Ask this agent a question...`}
            className="flex-1 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--accent-indigo)]"
          />
          <button
            onClick={handleCustomReason}
            disabled={streaming || !customPrompt.trim()}
            className="px-4 py-2 rounded-lg text-xs font-medium bg-[var(--accent-indigo)] text-white hover:opacity-90 disabled:opacity-50"
          >
            {streaming ? "Thinking..." : "Ask"}
          </button>
        </div>
        {streamOutput && (
          <div className="p-4 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-primary)] max-h-64 overflow-y-auto">
            <div className="text-xs text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap font-mono">
              {streamOutput}
              {streaming && <span className="w-2 h-4 inline-block bg-[var(--accent-indigo)] ml-0.5 status-pulse" />}
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
