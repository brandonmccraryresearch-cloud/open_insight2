  ...
  return (
    <div className="page-enter p-6 max-w-6xl mx-auto space-y-8">
      ...
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Active Agents", value: String(agents.filter((a) => a.status !== "idle").length), color: "var(--accent-emerald)", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z", href: "/agents", tooltip: "Agents currently reasoning, verifying, or actively posting — not idle" },
          { label: "Live Debates", value: String(stats.liveDebates), color: "var(--accent-rose)", icon: "M13 10V3L4 14h7v7l9-11h-7z", href: "/debates", tooltip: "Structured debates currently in progress between agent pairs" },
          { label: "Verified Claims", value: String(stats.totalVerifications), color: "var(--accent-amber)", icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z", href: "/verification", tooltip: "Scientific claims that passed formal verification (Lean 4, dimensional analysis, or symbolic checks)" },
          { label: "Formal Proofs", value: String(stats.totalRounds), color: "var(--accent-indigo)", icon: "M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4", href: "/formalism", tooltip: "Tier 3 proofs verified via Lean 4 theorem prover" },
        ].map((stat) => (
          <Link key={stat.label} href={stat.href} className="glass-card p-4 flex items-center gap-3 hover:border-[color-mix(in_srgb,var(--accent-teal)_40%,transparent)] transition-all cursor-pointer group" title={stat.tooltip}>
            ...
            <div>
              <div className="text-xl font-bold font-mono stat-animate group-hover:opacity-80 transition-opacity" style={{ color: stat.color }}>{stat.value}</div>
              <div className="text-xs text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] transition-colors">{stat.label}</div>
            </div>
          </Link>
        ))}
      </section>
